    ...
    try:
        ...
        if details in ['sessions', 'all']:
            ...
            for session in g.user.sessions:
                session_data = {'id': session.id, 'is_self_data': session.is_self_data, 'pos_count': session.positions.count(), 'session_name': session.session_name, 'coin_pair': session.coin_pair, 'timeframe': session.timeframe, 'additional_timaframe': session.additional_timaframe, 'cursor': session.cursor, 'balance': session.balance, 'current_PnL': session.current_PnL, 'positions': []}
